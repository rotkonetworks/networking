...
if [[ -z "$OUTPUT_DIR" ]]; then
  OUTPUT_DIR="${SCRIPT_DIR}/../config/hosts/${SITE}/systemd/network"
fi
mkdir -p "$OUTPUT_DIR"
...
UPLINKS=$(echo "$SITE_CONFIG" | jq -r '.physical_interfaces.uplinks[]?' 2>/dev/null | tr '\n' ' ')
...
P2P_VLANS=$(echo "$SITE_CONFIG" | jq -r '.physical_interfaces.p2p_vlans // false')
...
# Updated $(date -u +"%Y-%m-%d") for RR-based routing
...
# No default gateway here - use vmbr2 RR path
DNS=9.9.9.9
DNS=1.1.1.1"
...
[Route]
# Management routing table - backup default
...
[Route]
# Backup default in main table with high metric
Destination=0.0.0.0/0
Gateway=${MGMT_GATEWAY}
Metric=200
...
# Generate p2p VLAN configuration (separate links to each RR)
generate_p2p_vlans() {
  local uplink1 uplink2

  uplink1=$(echo "$UPLINKS" | awk '{print $1}')
  uplink2=$(echo "$UPLINKS" | awk '{print $2}')

  # Get p2p link configuration
  local rr1_vlan rr1_v4 rr1_v6 rr1_peer
  local rr2_vlan rr2_v4 rr2_v6 rr2_peer

  rr1_vlan=$(echo "$SITE_CONFIG" | jq -r '.p2p_links.rr1.vlan')
  rr1_v4=$(echo "$SITE_CONFIG" | jq -r '.p2p_links.rr1.v4')
  rr1_v6=$(echo "$SITE_CONFIG" | jq -r '.p2p_links.rr1.v6')
  rr1_peer=$(echo "$SITE_CONFIG" | jq -r '.p2p_links.rr1.peer')

  rr2_vlan=$(echo "$SITE_CONFIG" | jq -r '.p2p_links.rr2.vlan')
  rr2_v4=$(echo "$SITE_CONFIG" | jq -r '.p2p_links.rr2.v4')
  rr2_v6=$(echo "$SITE_CONFIG" | jq -r '.p2p_links.rr2.v6')
  rr2_peer=$(echo "$SITE_CONFIG" | jq -r '.p2p_links.rr2.peer')
  ...
  # Inner VLAN names
  local vlan_inner1="vlan${rr1_vlan}-p1"
  local vlan_inner2="vlan${rr2_vlan}-p2"
...
Id=${QINQ_OUTER}"
  ...
  write_file "22-vlan${QINQ_OUTER}-p1.network" "# Outer Q-in-Q VLAN on uplink 1 (${rr1_peer})
[Match]
Name=${vlan_outer1}
...
VLAN=${vlan_inner1}"
  ...
  write_file "22-vlan${QINQ_OUTER}-p2.network" "# Outer Q-in-Q VLAN on uplink 2 (${rr2_peer})
[Match]
Name=${vlan_outer2}
...
VLAN=${vlan_inner2}"
  ...
  # Inner VLAN netdev files (p2p links)
  write_file "23-vlan${rr1_vlan}-p1.netdev" "[NetDev]
Name=${vlan_inner1}
Kind=vlan
...
Id=${rr1_vlan}"
  ...
  write_file "23-vlan${rr2_vlan}-p2.netdev" "[NetDev]
Name=${vlan_inner2}
Kind=vlan
...
Id=${rr2_vlan}"
  ...
  # Inner VLAN network files - p2p links with addresses
  write_file "24-vlan${rr1_vlan}-p1.network" "# Point-to-point link to RR1 (${rr1_peer})
[Match]
Name=${vlan_inner1}
...
IPv6AcceptRA=false"
  ...
  write_file "24-vlan${rr2_vlan}-p2.network" "# Point-to-point link to RR2 (${rr2_peer})
[Match]
Name=${vlan_inner2}
...
# Generate public bridge (vmbr2 - BGP RR network) - only for bonded mode
generate_vmbr2() {
...
if [[ "$P2P_VLANS" == "true" ]]; then
  echo "  Mode: p2p VLANs (separate links to each RR)"
  generate_p2p_vlans
elif [[ "$BONDED_VLANS" == "true" ]]; then
  echo "  Mode: bonded VLANs (bond with vmbr2 bridge)"
  generate_bonded_vlans
  generate_vmbr2
fi
