# 2025-05-08 03:02:33 by RouterOS 7.19beta8
...
/routing bgp template add afi=ipv6 as=142108 input.filter=iBGP-IN multihop=yes name=IBGP-ROTKO-v6 nexthop-choice=default output.filter-chain=iBGP-OUT .network=ipv6-apnic-rotko router-id=10.155.255.4
...
/ip firewall raw add action=drop chain=prerouting comment="BCP214: Block BGP IPv4 AMSIX - src to dst" disabled=yes dst-address=80.249.208.0/21 dst-port=179 protocol=tcp src-address=80.249.208.0/21
/ip firewall raw add action=drop chain=prerouting comment="BCP214: Block BGP IPv4 AMSIX - dst to src" disabled=yes dst-address=80.249.208.0/21 protocol=tcp src-address=80.249.208.0/21 src-port=179
...
/routing bgp connection add input.limit-process-routes-ipv4=2000000 local.address=10.155.255.4 .role=ibgp multihop=yes name=ROTKO-BKK00-TO-BKK20-v4 nexthop-choice=default output.keep-sent-attributes=yes .redistribute=connected,bgp remote.address=10.155.255.2 .as=142108 templates=IBGP-ROTKO-v4
...
/routing filter rule add chain=HGC-SG-OUT-v4 rule="set bgp-med 50; set bgp-path-prepend 3; set bgp-large-communities location; accept"
/routing filter rule add chain=HGC-SG-OUT-v4 rule="set bgp-med 50; set bgp-path-prepend 3; set bgp-large-communities location; accept"
/routing filter rule add chain=HGC-SG-OUT-v6 rule="set bgp-med 50; set bgp-path-prepend 3; set bgp-large-communities location; accept"
/routing filter rule add chain=HGC-SG-OUT-v6 rule="set bgp-med 50; set bgp-path-prepend 3; set bgp-large-communities location; accept"
