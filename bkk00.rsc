# 2025-04-23 08:33:42 by RouterOS 7.19beta8
# software id = 61HF-9FEH
#
# model = CCR2216-1G-12XS-2XQ
# serial number = HH40ADXHPY7
/interface bridge add name=bridge_local vlan-filtering=yes
/interface ethernet set [ find default-name=sfp28-1 ] comment=BKK10-LAG
...
/interface ethernet set [ find default-name=sfp28-3 ] comment=BKK50-LAG
...
/interface bonding add comment=bkk10-sfp28-1 lacp-rate=1sec mode=802.3ad name=BKK10-LAG slaves=sfp28-1 transmit-hash-policy=layer-2-and-3
...
/interface bonding add comment=bkk50-sfp28-3 lacp-rate=1sec mode=802.3ad name=BKK50-LAG slaves=sfp28-3 transmit-hash-policy=layer-2-and-3
...
/routing bgp template add afi=ipv6 as=142108 input.filter=iBGP-IN multihop=yes name=IBGP-ROTKO-v6 nexthop-choice=propagate output.filter-chain=iBGP-OUT .network=ipv6-apnic-rotko router-id=10.155.255.4
...
/routing ospf instance add disabled=no name=ospf-instance-v3 originate-default=always router-id=10.155.255.4
...
/routing bgp template set default afi=ip as=142108 comment=IBGP-ROTKO-v4 input.filter=iBGP-IN multihop=yes nexthop-choice=propagate output.filter-chain=iBGP-OUT .network=ipv4-apnic-rotko router-id=10.155.255.4 routing-table=main
...
/ip address add address=172.16.40.1/30 interface=BKK10-LAG network=172.16.40.0
/ip address add address=172.16.50.1/30 interface=BKK50-LAG network=172.16.50.0
...
/ip firewall raw add action=drop chain=prerouting comment="BCP38 - Filter spoofed source addresses" in-interface-list=WAN src-address-list=our-networks
...
/ip route add disabled=no distance=220 dst-address=0.0.0.0/0 gateway=BKK20-LAG pref-src=160.22.181.180
...
/ipv6 address add address=fd00:dead:beef:40::1/126 advertise=no interface=BKK10-LAG
/ipv6 address add address=fd00:dead:beef:50::1/126 advertise=no interface=BKK50-LAG
...
/ipv6 firewall raw add action=accept chain=prerouting comment="Allow BGP loopback traffic SOURCE" src-address-list=bgp-loopback-ips
/ipv6 firewall raw add action=accept chain=prerouting comment="Allow BGP loopback traffic DEST" dst-address-list=bgp-loopback-ips
/ipv6 firewall raw add action=accept chain=prerouting comment="Allow IPv6 loopback traffic" src-address=::1/128
/ipv6 firewall raw add action=accept chain=prerouting comment="Allow IPv6 loopback traffic" dst-address=::1/128
...
/routing bgp connection add input.limit-process-routes-ipv4=2000000 local.address=10.155.255.4 .role=ibgp multihop=yes name=ROTKO-BKK00-TO-BKK20-v4 nexthop-choice=default output.keep-sent-attributes=yes .redistribute=connected,bgp remote.address=10.155.255.2 .as=142108 templates=default
/routing bgp connection add afi=ipv6 as=142108 disabled=no input.filter=iBGP-IN .limit-process-routes-ipv6=2000000 local.address=fd00:dead:beef::100 .role=ibgp multihop=yes name=ROTKO-BKK00-TO-BKK20-v6 nexthop-choice=default output.filter-chain=iBGP-OUT .keep-sent-attributes=yes .redistribute=connected,bgp remote.address=fd00:dead:beef::20 .as=142108 router-id=10.155.255.4 templates=IBGP-ROTKO-v6
...
/routing bgp connection add disabled=no input.filter=ROUTEVIEWS-IN-v4 .limit-process-routes-ipv4=10 local.role=ebgp name=RouteViews-BKNIX-v4 output.filter-chain=ROUTEVIEWS-OUT-v4 remote.address=203.159.68.20 .as=6447 templates=BKNIX-v4
/routing bgp connection add disabled=no input.filter=ROUTEVIEWS-IN-v6 .limit-process-routes-ipv6=10 local.role=ebgp name=RouteViews-BKNIX-v6 output.filter-chain=ROUTEVIEWS-OUT-v6 remote.address=2001:df5:b881::20 .as=6447 templates=BKNIX-v6
...
/routing ospf interface-template add area=backbone comment=BKK50-v4 disabled=no networks=172.16.50.0/30
...
/routing ospf interface-template add area=backbone-v6 comment="ULA BKK50 Link" disabled=no networks=fd00:dead:beef:50::1/126
...
/system script add dont-require-permissions=no name=graceful_shutdown_on owner=admin policy=ftp,reboot,read,write,policy,test,password,sniff,sensitive,romon source="\
    ...
    \n"
/system script add dont-require-permissions=no name=graceful_shutdown_off owner=admin policy=ftp,reboot,read,write,policy,test,password,sniff,sensitive,romon source="\
    \n    # BKNIX IPv4 and IPv6 prefixes\
    \n    :local v4Prefix \"203.159.68.0/23\"\
    \n    :local v6Prefix \"2001:df5:b881::/64\"\
    \n    \
    ...
    \n"
/system script add dont-require-permissions=no name=bknix_maintenance_on owner=admin policy=ftp,reboot,read,write,policy,test,password,sniff,sensitive,romon source="\
    \n    # BKNIX IPv4 and IPv6 prefixes\
    \n    :local v4Prefix \"203.159.68.0/23\"\
    \n    :local v6Prefix \"2001:df5:b881::/64\"\
    \n    :local interface \"BKNIX-LAG\"\
    \n    \
    \n    :log info \"Enabling BGP Session Culling (BCP 214) maintenance mode for \$interface\"\
    \n    :log info \"Using IPv4 prefix: \$v4Prefix and IPv6 prefix: \$v6Prefix\"\
    \n    \
    \n    # Enable graceful shutdown first (5 min before)\
    \n    :log info \"Enabling graceful shutdown for BGP sessions (RFC 8326)\"\
    \n    /system script run graceful_shutdown_on\
    \n    \
    \n    # Wait for 5 minutes\
    \n    :log info \"Waiting 5 minutes for graceful shutdown to take effect\"\
    \n    :delay 5m\
    \n    \
    \n    # Create IPv4 filter to block BGP traffic\
    \n    /ip firewall filter\
    \n    add chain=forward action=drop protocol=tcp src-address=\$v4Prefix dst-address=\$v4Prefix dst-port=179 comment=\"BCP214: Block BGP IPv4 - src to dst\"\
    \n    add chain=forward action=drop protocol=tcp src-address=\$v4Prefix dst-address=\$v4Prefix src-port=179 comment=\"BCP214: Block BGP IPv4 - dst to src\"\
    \n    \
    \n    # Create IPv6 filter to block BGP traffic\
    \n    /ipv6 firewall filter\
    \n    add chain=forward action=drop protocol=tcp src-address=\$v6Prefix dst-address=\$v6Prefix dst-port=179 comment=\"BCP214: Block BGP IPv6 - src to dst\"\
    \n    add chain=forward action=drop protocol=tcp src-address=\$v6Prefix dst-address=\$v6Prefix src-port=179 comment=\"BCP214: Block BGP IPv6 - dst to src\"\
    \n    \
    \n    :log info \"BGP Session Culling maintenance mode ENABLED - all BGP sessions on \$interface will be torn down\"\
    \n    :log info \"You can now safely perform maintenance activities on the BKNIX-LAG interface\"\
    \n"
/system script add dont-require-permissions=no name=bknix_maintenance_off owner=admin policy=ftp,reboot,read,write,policy,test,password,sniff,sensitive,romon source="# BKNIX IPv4 and IPv6 prefixes\r\
    \n:local v4Prefix \"203.159.68.0/23\"\r\
    \n:local v6Prefix \"2001:df5:b881::/64\"\r\
    \n\r\
    \n:log info \"Disabling BGP Session Culling (BCP 214) maintenance mode\"\r\
    \n\r\
    \n# Remove IPv4 firewall filters for BGP traffic\r\
    \n:foreach rule in=[/ip firewall filter find comment~\"BCP214:\"] do={\r\
    \n/ip firewall filter remove \$rule\r\
    \n}\r\
    \n\r\
    \n# Remove IPv6 firewall filters for BGP traffic\r\
    \n:foreach rule in=[/ipv6 firewall filter find comment~\"BCP214:\"] do={\r\
    \n/ipv6 firewall filter remove \$rule\r\
    \n}\r\
    \n\r\
    \n# Disable graceful shutdown\r\
    \n/system script run graceful_shutdown_off\r\
    \n\r\
    \n# Reset BGP connections to ensure they reconnect properly\r\
    \n:foreach conn in=[/routing bgp connection find name~\"BKNIX\"] do={\r\
    \n:local connName [/routing bgp connection get \$conn name]\r\
    \n# Try to reset the session\r\
    \n:log info \"Resetting BGP connection \$connName\"\r\
    \n/routing bgp connection reset \$conn\r\
    \n}\r\
    \n\r\
    \n:log info \"BGP Session Culling maintenance mode DISABLED - BGP sessions will be restored\"\r\
    \n"
